...
const BaseMonitor: React.FC = () => {
  ...
  const handleTrade = async (token: BaseToken) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        const result = await executeBaseTrade(
          token.address,
  ...
  const columns: ColumnsType<BaseToken> = [
    {
      title: '代币',
      dataIndex: 'symbol',
      key: 'symbol',
